...
class CoopForm(models.Model):
    ...
    society_category = models.CharField(('Category of the Society'), choices=SOCIETY_CATEGORY,
                                        max_length=50, default='Arts and Science Society', blank=False)
    ...
    society_lga_of_activities = models.CharField(
        'LGA of Activities', choices=LGA, max_length=30, default='Akoko-Edo', blank=False)

    def get_upload_path(self, filename):
        return "images/{0}/{1}".format(self.society_name, filename)

    # Logo
    society_logo = models.ImageField("Society's Logo", upload_to=get_upload_path,
                                     default=f"{settings.FORM_STATIC}img/default.png", blank=True)
    ...
    first_purpose = models.CharField(
        ('First Purpose'), max_length=300, blank=False)
    second_purpose = models.CharField(
        ('Second Purpose'), max_length=300, blank=False)
    third_purpose = models.CharField(
        ('Third Purpose'), max_length=300, blank=False)
    ...
    nature_registration = models.CharField(
        ('Nature of Registration'), choices=REGISTRATION_TYPE, max_length=100, blank=False)
    
    def get_upload_path(self, filename):
        return "documents/{0}/{1}".format(
            self.society_name, filename
        )
    ...
    attach_first_minute = models.FileField(
        'Attach Minute of First Meeting', upload_to=get_upload_path, blank=False,
        validators=[FileExtensionValidator(['pdf', 'doc', 'docx', 'txt', 'png', 'jpg'])])
    attach_second_minute = models.FileField(
        'Attach Minute of Second Meeting', upload_to=get_upload_path, blank=False,
        validators=[FileExtensionValidator(['pdf', 'doc', 'docx', 'txt', 'png', 'jpg'])])

    # 10 Members in the Society
    president_name = models.CharField(
        "President's Name", max_length=100, blank=False)
    ...
    secretary_name = models.CharField(
        "Secretary's Name", max_length=100, blank=False)
    secretary_position = models.CharField(
        "Secretary's Position", max_length=100, blank=True, default="Secretary")
    secretary_phone_number = PhoneNumberField(
        "Secretary's Phone Number", blank=False)

    treasurer_name = models.CharField(
        "Treasurer's Name", max_length=100, blank=False)
    treasurer_position = models.CharField(
        "Treasurer's Position", max_length=100, blank=True, default="Treasurer")
    treasurer_phone_number = PhoneNumberField(
        "Treasurer's Phone Number", blank=False)
    ...
    have_bye_laws = models.CharField(max_length=128, choices=BYE_LAWS_CHOICES, blank=False, default='No')

    # Attach bye-laws
    attach_bye_laws = models.FileField(
        ('Attach a copy of your Bye-Laws'), upload_to=get_upload_path, blank=True,
        validators=[FileExtensionValidator(['pdf', 'doc', 'docx', 'png', 'txt', 'jpg'])])
...
class shareCapitalFiles(models.Model):
    ...
    attach_share_capital = models.FileField(
        ('Attach Share Capital'), upload_to=get_upload_path, blank=True,
        validators=[FileExtensionValidator(['pdf', 'doc', 'docx', 'txt', 'png', 'jpg'])])
