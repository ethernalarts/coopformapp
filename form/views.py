...
class CoopLoginView(LoginView):
    ...
    def get_success_url(self):
        return (
            ...
            or self.request.POST.get("previous_page")
            or self.get_default_redirect_url()
        )
...
class RegisterView(LoginRequiredMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST.get(
            "nature_registration"
        ) == "Limited Liability" and not self.request.FILES.getlist(
            "attach_share_capital"
        ):
            messages.error(
                self.request,
                "Please attach share capital files or select a different option",
            )
            return self.form_invalid(form)
        ...
        if self.request.POST.get(
            "have_bye_laws"
        ) == "Yes" and not self.request.FILES.get("attach_bye_laws"):
            messages.error(
                self.request,
                "Please attach a copy of your bye laws or select a different option",
            )
            return self.form_invalid(form)
...
class updateFormView(LoginRequiredMixin, UpdateView):
    ...
    def get_share_capital_files(self):
        return shareCapitalFiles.objects.filter(
    ...
    def checks(self):
        ...
        if (
            self.request.POST.get("have_bye_laws") == "No"
            and self.object.attach_bye_laws
        ):
            self.object.attach_bye_laws.delete()
            messages.success(self.request, "Bye Laws file has been deleted")
    ...
    def post(self, *args, **kwargs):
        ...
        if self.request.POST.get("nature_registration") == "Limited Liability":
            if not self.get_share_capital_files() and not files:
                messages.error(
                    self.request,
                    "Please attach share capital files (5a) or choose a different option",
                )
                return self.form_invalid(form)

        # check if yes is selected and a copy of bye-laws is attached
        if self.request.POST.get("have_bye_laws") == "Yes":
            if not self.object.attach_bye_laws and not self.request.FILES.get(
                "attach_bye_laws"
            ):
                messages.error(
                    self.request,
                    "Please attach a copy of your bye laws (9a) or choose a different option",
                )
                return self.form_invalid(form)
...
class formDetailsView(DetailView):
    ...
    def get_share_capital_files(self):
        return shareCapitalFiles.objects.filter(coopform_id=self.object.id).order_by(
            "id"
        )
        ...
        if query := self.request.GET.get("q"):
            ...
            for q in queries:
                results = CoopForm.objects.filter(
                    Q(society_name__icontains=q) | Q(society_category__icontains=q)
                ).distinct()

                queryset.extend(iter(results))
            return sorted(
                list(set(queryset)), key=attrgetter("society_name"), reverse=True
            )
        else:
...
def sendmail(request, context, template_name):
    ...
    msg = mail.EmailMultiAlternatives(subject, text_content, from_email, recipient_list)
    ...
    return HttpResponse(
        template.render({"society_name": context.get("society_name")}, request)
    )
...
def generate_pdf(request, pk):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="{society_name}_profile_printout.pdf"'
    )
